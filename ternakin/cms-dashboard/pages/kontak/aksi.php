<?php 
	...
	switch ($aksi) {
		case 'tambah':
			...
			$direktori   = "../../../assets/image/kontak/$nama_file";

			if (!empty($lokasi_file)) {
				if (move_uploaded_file($lokasi_file,$direktori)) {
							
					$query_tambah	= mysqli_query($con,"INSERT INTO tb_kontak VALUES ('',  '$nama',  '$nama_file')");
		...
		case 'update-kontak':
			...
			if (!empty($lokasi_file)) {
				if (move_uploaded_file($lokasi_file,$direktori)) {
					...
					unlink("../../../assets/image/kontak/$nama_file".$data_file_gambar_lama['icon']);
