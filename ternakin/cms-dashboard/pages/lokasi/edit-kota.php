  ...
  <?php 
    ...
    $sqlProv = mysqli_query($con,"SELECT * FROM tb_kota WHERE id_kota='".$_GET['id_kota']."'");
    $dataKota = mysqli_fetch_assoc($sqlProv);
  ?>
  <!-- End Query -->
</head>
                ...
                <div class="card-body">
                  <form method="POST" action="<?=$_ENV['base_url']?>cms-dashboard/pages/lokasi/aksi">
                    <div class="form-group">
                      <input type="hidden" name="id_kotaz" value="<?=$dataKota['id_kota']?>">
                      <label>id_kota</label>
                      <input type="number" name="id_kota" id="id_kota" class="form-control" readonly value="<?=$dataKota['id_kota']?>">
                    </div>
                    <div class="form-group">
                      ...
                      <select class="form-control selectpicker" name="provinsi" id="provinsi" data-live-search="true">
                        ...
                        <?php 
                          while ($data = mysqli_fetch_array($sql)) {
                            ?>
                              <option value="<?=$data['id_provinsi']?>" <?=($dataKota['id_provinsi']==$data['id_provinsi'])?'selected':'';?>><?=$data['nama_provinsi']?></option>
                            <?php
                          }
                         ?>
                      </select>
                    ...
                    <div class="form-group">
                      <label>Kota</label>
                      <input type="text" name="kota" class="form-control" value="<?=$dataKota['nama_kota']?>">
                    </div>
                    <button type="submit" name="aksi" class="btn btn-primary" value="edit-kota">edit Kota</button>
                  </form>
                </div>
              </div>
        ...
        $("#provinsi").on('change',function(){
            ...
            $.ajax({
              ...
              success:function(data){
                console.log(data)
                if (data.id_kota==<?=$_GET['id_kota']?>) {
                    $("#id_kota").val(<?=$_GET['id_kota']?>)
                }else{
